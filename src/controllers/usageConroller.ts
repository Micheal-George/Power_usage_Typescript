...
export const createUsage: RequestHandler = async (req, res, next) => {
  ...
  let str: string = "";
  if (min < 10 && num < 10) str += "0" + num + ":" + "0" + min + ":" + "00";
  else if (min >= 10 && num < 10) str += "0" + num + ":" + min + ":" + "00";
  else if (min < 10 && num >= 10) str += num + ":" + "0" + min + ":" + "00";
  const v = req.body;
  let c: number = 0;
  if (v.applianceType == "low-power") c += 2 * num;
  if (v.applianceType == "mid-power") c += 3 * num;
  if (v.applianceType == "high-power") c += 4 * num;
  ...
  var Usage = await Powers.create({
    fromTime: v.fromTime,
    toTime: v.toTime,
    applianceType: v.applianceType,
    unitConsumed: c,
    duration: str,
    userId: normalid,
  });
...
export const getAllUsage: RequestHandler = async (req, res, next) => {
  ...
  if (allUsage.length == 0) {
    return res.status(400).json({ message: "No usage data", data: allUsage });
  }
...
export const getUsageById: RequestHandler = async (req, res, next) => {
  ...
  if (Usage == null) {
    return res.status(400).json({ message: "No usage data", data: Usage });
  }
  return res
    .status(200)
